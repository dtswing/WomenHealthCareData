...
train.combined.v2 <- train.combined[,colSums(is.na(train.combined)| train.combined == '') < nrow(train.combined)*0.6] 
...
fit_list <-cbind(fit_list, rf_fit)
pred_train_all <- predict(rf_fit, train.set, type = "prob")[,2]
pred_obj_train_all <- prediction(pred_train_all, train.set$service_a)
auc_train_all <- round(performance(pred_obj_train_all, "auc")@y.values[[1]], digits=3)
y_pred <- cbind(y_pred, pred_obj_train_all)
